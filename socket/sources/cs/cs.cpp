...
CS::CS(::QWidget *parent)
  ...
  auto general_layout = new ::QVBoxLayout;
  {
    ...
    auto connecting_layout = new ::QHBoxLayout;
    {
      ...
      connect(connect_button, SIGNAL(clicked()), ip_edit, SIGNAL(returnPressed()));
    ...
    general_layout->addLayout(connecting_layout);
  }
...
void CS::read_message() { text_edit_->append("NM: " + socket_->readAll()); }

void CS::listen_slot() {
  if (!server_) {
    text_edit_->append("__SERVER__");
    server_ = new ::QTcpServer{this};
    server_->listen(/*::QHostAddress::AnyIPv4*/::QHostAddress::Any, default_port);
